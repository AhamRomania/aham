...
if [ "$answer" == "y" ]; then
    ...
    cd $ROOT_DIR/backend/service/url/ && env CGO_ENABLED=1 GOOS=linux GARCH=amd64 go build -o $BIN_PATH/url -tags "libsqlite3 linux" -a -ldflags '-extldflags "-static"' main.go
