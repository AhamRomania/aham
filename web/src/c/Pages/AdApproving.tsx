...
const AdApproving: FC = ({}) => {
  ...
  return (
    <>
        ...
        <div>
        {ads.map((item, index) => (
            <AdApprovingItem onTouched={() => update()} key={index} ad={item} />
        ))}
        </div>
    </>
  );
...
const AdApprovingItem: FC<AdApprovingItemProps> = ({ ad, onTouched }) => {
  ...
  const publish = () => {
    if (confirm("Publish " + ad.title + "?")) {
      api(`/ads/${ad.id}/publish`, { method: "POST", success: true })
        .then(() => {
          onTouched();
        })
        .catch(() => {
  ...
  const reject = () => {
    if (confirm("Reject " + ad.title + "?")) {
      api(`/ads/${ad.id}/reject`, { method: "POST", success: true })
        .then(() => {
          onTouched();
        })
        .catch(() => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Button size="lg" variant="solid" onClick={() => publish()}>
            <Publish />
        ...
        <div>
          <Modal open={true} onClose={() => setRequestingUpdate(false)}>
            <ModalDialog>
              ...
              <DialogActions>
                <Button onClick={() => setRequestingUpdate(false)}>
                  Request
                </Button>
              </DialogActions>
            </ModalDialog>
